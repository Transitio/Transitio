...
/// }
/// </code>
/// By default, every validator on a property runs on each <see cref="Validate(T)"/> call
/// (<see cref="CascadeMode.Continue"/>), so every failure is reported at once. Call
/// <c>.Cascade(CascadeMode.StopOnFirstFailure)</c> on a rule to stop at its first failure instead.
/// </summary>
