...
and this project adheres to [Semantic Versioning](Semantic Versioning 2.0.0).
 
## [1.3.0] - 2026-09-12
 
### Added
- **`Transitio.Mediator`** — open-generic pipeline behaviors (e.g. `LoggingBehavior<TRequest, TResponse>`)
  are now discovered by `AddTransitioMediator`'s assembly scan and registered against the open
  `IPipelineBehavior<,>` service type, so they no longer require manual registration.
- **`Transitio.Validation`** — `CascadeMode.StopOnFirstFailure`, set per property with
  `RuleFor(...).Cascade(CascadeMode.StopOnFirstFailure)`, stops running validators for that
  property after the first failure instead of collecting them all.
- **`Transitio.Mapper`** — `Map(source, destination)` maps a source object onto an existing
  destination instance in place (useful for updating an already-tracked entity/DTO), instead of
  always allocating a new one. `ForMember`/`Ignore`/`Condition` behave the same as
  `Map<TDestination>(source)`; a `ConvertUsing` converter configured for the pair is not supported
  by this overload and throws.
- **`Transitio.Dependency`** — `TryAddTransitio`/`TryAddKeyedTransitio` mirror every
  `AddTransitio`/`AddKeyedTransitio` overload, but only register if `IMapper` (or, for the keyed
  variant, the given key) is not already registered — the first call wins, for composing modules
  that may each want to ensure Transitio is available.
- **`Transitio.Assertions`** — dictionary assertions (`ContainKey`, `NotContainKey`, `ContainValue`,
  `NotContainValue`) and `Guid`/`Guid?` support for `.Should()`.
...
[1.3.0]: Comparing v1.2.0...HEAD · Transitio/Transitio
[1.2.0]: Comparing v1.1.0...v1.2.0 · Transitio/Transitio
[1.1.0]: Comparing v1.0.8...v1.1.0 · Transitio/Transitio
[1.0.8]: Comparing v1.0.7...v1.0.8 · Transitio/Transitio  
[1.0.7]: Comparing v1.0.6...v1.0.7 · Transitio/Transitio  
[1.0.6]: Comparing v1.0.5...v1.0.6 · Transitio/Transitio  
[1.0.5]: Comparing v1.0.4...v1.0.5 · Transitio/Transitio  
[1.0.4]: Comparing v1.0.2...v1.0.3 · Transitio/Transitio  
