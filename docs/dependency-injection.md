...
## Idempotent registration – `TryAddTransitio`
 
`TryAddTransitio`/`TryAddKeyedTransitio` mirror every `AddTransitio`/`AddKeyedTransitio` overload,
but only register if `IMapper` (or, for the keyed variant, the given key) is not already
registered:
 
```csharp
// ModuleA and ModuleB both want to guarantee Transitio is available, without stomping on
// whichever one runs first:
services.TryAddTransitio(cfg => cfg.CreateMap<User, UserDto>());   // wins if called first
services.TryAddTransitio(cfg => cfg.CreateMap<Order, OrderDto>()); // skipped if IMapper is already registered
```
 
> **First call wins, not last.** This is the opposite of `AddTransitio`, which stacks
> registrations so the *last* call's configuration is what `GetRequiredService<IMapper>()`
> resolves. With `TryAddTransitio`, if `IMapper` (or, for keyed, that key) is already registered,
> a later call's configuration delegate is **not built at all** — it's skipped entirely, including
> any side effects and `ValidateConfiguration()` checks, so one module's unused (even invalid)
> configuration can never break another module's already-registered mapper.
> Use `TryAddTransitio` when composing modules that may each want to ensure Transitio is
> available without knowing whether another module already configured it.
