...
**Open-generic** pipeline behaviors (e.g. `LoggingBehavior<,>`) are discovered automatically too —
the scan registers them against the open `IPipelineBehavior<,>` service type, so the container
builds a closed behavior for whatever request/response pair is resolved:
 
```csharp
services.AddTransitioMediator(typeof(PingHandler).Assembly); // LoggingBehavior<,> is discovered automatically
```
 
Only behaviors generic in **both** type parameters are auto-registered this way. A behavior that's
only partially generic (e.g. fixed to a specific response type) is not discovered and needs manual
registration. Closed behaviors from the same scan always wrap open-generic ones from the outside —
this ordering comes from the two-pass scan itself and isn't independently configurable.
 
Manual registration still works if you'd rather be explicit, or need a specific behavior on a
different lifetime than the rest of the scan:
 
```csharp
services.AddTransient(typeof(IPipelineBehavior<,>), typeof(LoggingBehavior<,>));
```
 
Open-generic `IRequestHandler<,>`/`INotificationHandler<>` implementations are not discovered by
this scan — only pipeline behaviors.
