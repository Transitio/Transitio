...
> This resolves for `IDictionary<TKey,TValue>` (including concrete `Dictionary<TKey,TValue>`)
> subjects specifically. A variable statically typed as `IReadOnlyDictionary<TKey,TValue>` falls
> through to the generic collection assertions instead, since `IReadOnlyDictionary` doesn't
> implement `IDictionary`.
 
## Guid
 
`Guid`/`Guid?` get `Be`/`NotBe`/`BeNull`/`NotBeNull` only — no ordering (`BeGreaterThan`/`BeInRange`),
since Guid ordering isn't meaningful, and no `BeSameAs`/`NotBeSameAs`, since a boxed Guid's
reference never reflects value equality (it would always fail for equal values and always pass for
unequal ones, regardless of the actual Guid).
