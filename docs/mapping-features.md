...
## Mapping into an existing instance – `Map(source, destination)`
 
```csharp
var dto = new UserDto();
mapper.Map(user, dto); // populates dto in place and returns it
 
// Useful for updating an already-tracked entity/DTO without allocating a new one:
mapper.Map(updateRequest, existingEntity);
```
 
With `SetIgnoreNullValues(true)`, a null-sourced property preserves whatever value was already on
the destination instance, rather than falling back to a type default (there's no freshly
constructed instance to fall back to here, unlike `Map<TDestination>(source)`).
 
> Known limitations (v1): a `ConvertUsing` converter configured for the pair is not supported by
> this overload and throws `InvalidOperationException` — use `Map<TDestination>(source)` for
> converter-backed maps. Nested object/collection properties are still freshly allocated; only the
