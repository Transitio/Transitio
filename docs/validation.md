...
By default, every validator on a property runs on each `Validate` call (a **Continue** cascade), so
all failures are reported at once rather than stopping at the first. Override this per property
with `.Cascade(...)` — see [Cascade mode](#cascade-mode).
...
By default a property's validators all run and every failure is collected (`CascadeMode.Continue`).
Call `.Cascade(CascadeMode.StopOnFirstFailure)` to stop at the first failing validator for that
property instead — it can appear anywhere in the chain and always governs the whole property:
 
```csharp
RuleFor(c => c.Name)
    .Cascade(CascadeMode.StopOnFirstFailure)
    .NotEmpty().WithMessage("Name is required")
    .MaximumLength(50); // only runs if NotEmpty passed
```
...
The following are planned for a later release: `RuleForEach` for collection elements,
child-validator composition (`SetValidator`), asynchronous validators, severity levels, and
